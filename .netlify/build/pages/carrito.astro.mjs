/* empty css                                   */
import { d as createComponent, e as createAstro, m as maybeRenderHead, j as renderScript, g as addAttribute, r as renderTemplate, k as renderComponent } from '../chunks/astro/server_BcgSpGfh.mjs';
import 'piccolore';
import { $ as $$Layout } from '../chunks/Layout_BW__mhZt.mjs';
import { $ as $$BoutiqueCard } from '../chunks/BoutiqueCard_CXAY1ZBC.mjs';
import 'clsx';
import { c as catalogData } from '../chunks/catalog_rU3iDNxw.mjs';
...
Datos de Despacho
</h3> <div class="col-span-1 md:col-span-2 space-y-4"> <input type="text" name="name" placeholder="Nombre completo" required class="w-full bg-white/5 border border-white/20 rounded-lg px-4 py-2 text-white focus:border-honey-bronze-400 outline-none"> <p class="text-red-400 text-sm mt-1 hidden error-msg" id="error-name"></p> <input type="text" name="rut" id="input-rut" placeholder="RUT (12.345.678-9)" required class="w-full bg-white/5 border border-white/20 rounded-lg px-4 py-2 text-white focus:border-honey-bronze-400 outline-none"> <p class="text-red-400 text-sm mt-1 hidden error-msg" id="error-rut"></p> <input type="email" name="email" placeholder="Tu email" required class="w-full bg-white/5 border border-white/20 rounded-lg px-4 py-2 text-white focus:border-honey-bronze-400 outline-none"> <p class="text-red-400 text-sm mt-1 hidden error-msg" id="error-email"></p> <input type="tel" name="phone" placeholder="Teléfono (+569...)" required class="w-full bg-white/5 border border-white/20 rounded-lg px-4 py-2 text-white focus:border-honey-bronze-400 outline-none"> <p class="text-red-400 text-sm mt-1 hidden error-msg" id="error-phone"></p> <input type="text" name="commune" placeholder="Tu comuna" required class="w-full bg-white/5 border border-white/20 rounded-lg px-4 py-2 text-white focus:border-honey-bronze-400 outline-none"> <p class="text-red-400 text-sm mt-1 hidden error-msg" id="error-commune"></p> <input type="text" name="address" placeholder="A cual dirección deseas que llegue tu pedido?" required class="w-full bg-white/5 border border-white/20 rounded-lg px-4 py-2 text-white focus:border-honey-bronze-400 outline-none"> <p class="text-red-400 text-sm mt-1 hidden error-msg" id="error-address"></p> <div class="col-span-1 md:col-span-2 mt-6 pt-6 border-t border-white/20"> <h3 class="text-xl font-serif text-honey-bronze-100 mb-4">
Registra una fecha importante para ti, para acceder a
                    descuentos
</h3> <h5 class="text-sm font-serif text-honey-bronze-100 mb-2">
Por ejemplo: Cumpleaños, Aniversario, etc.
</h5> <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-white/5 p-4 rounded-lg border border-white/10"> <div> <input type="date" name="important_date" placeholder="Fecha importante" required class="w-full bg-white/5 border border-white/20 rounded-lg px-4 py-2 text-white focus:border-honey-bronze-400 outline-none"> <p class="text-red-400 text-sm mt-1 hidden error-msg" id="error-important_date"></p> </div> <div> <input type="text" name="reason" placeholder="Motivo (ej: Cumpleaños)" required class="w-full bg-white/5 border border-white/20 rounded-lg px-4 py-2 text-white focus:border-honey-bronze-400 outline-none"> <p class="text-red-400 text-sm mt-1 hidden error-msg" id="error-reason"></p> </div> </div> </div> <div class="col-span-1 md:col-span-2 flex justify-end gap-4 mt-4"> <button type="button" id="btn-cancel" class="px-6 py-3 border border-red-500/50 text-red-300 rounded-lg hover:bg-red-500/10 transition-colors">
Cancelar Compra
